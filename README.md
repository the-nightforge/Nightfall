...
| 💬 | **Full chat reveal** at game over — the wolves' den and the dead's channel open up once roles are public, and the whole match is loaded from the stored transcript |
...
- **The full end-of-match reveal depends on the database write.** At `GAME_OVER` the room screen swaps the snapshot's last 60 messages for the stored transcript (up to 2000 messages), fetched with the `gameId` the snapshot carries. The transcript is written asynchronously, so the client retries briefly; if the write fails, the reveal falls back to those last 60 messages.

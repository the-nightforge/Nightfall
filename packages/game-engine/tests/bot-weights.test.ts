...
describe("v2 là cấu hình production", () => {
  ...
  it("mặc định trỏ tới v16", () => {
    ...
    // khi làng đã mỏng, v16 cho Sói cãi giả; v2-v4 vẫn tồn tại nguyên vẹn làm
    // mốc so sánh.
    expect(DEFAULT_BOT_WEIGHTS.version).toBe("16.0.0");
    expect(weightsPreset("16.0.0")).toBe(DEFAULT_BOT_WEIGHTS);
    expect(weightsPreset("15.0.0")).toBe(BOT_WEIGHTS_V15);
    expect(weightsPreset("14.0.0")).toBe(BOT_WEIGHTS_V14);
    expect(weightsPreset("13.0.0")).toBe(BOT_WEIGHTS_V13);
    expect(weightsPreset("12.0.0")).toBe(BOT_WEIGHTS_V12);

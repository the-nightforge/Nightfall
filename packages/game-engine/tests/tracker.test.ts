...
describe("kết quả theo dõi ở bình minh", () => {
  ...
  it("Bảo Vệ có che ai đó cũng đọc ra ĐÃ ra tay", () => {
    // GUARD không nộp vào bất kỳ `Record` chung nào (khác Sói, Tiên Tri, ...) -
    // `didActTonight` phải tra riêng `guardTarget` qua ánh xạ vai, giống hệt
    // cách `nightInfo` đọc "acted" cho chính người này.
    const engine = trackerGame();
    engine.state.players[3].role = "GUARD";
    engine.submitNightAction("t1", "TRACK", "v1");
    engine.submitNightAction("v1", "GUARD", "v2");
    engine.submitNightAction("w1", "KILL", "s1");
    engine.resolveNight(Date.now(), () => 0);

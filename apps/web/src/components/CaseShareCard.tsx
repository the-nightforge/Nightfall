...
export function CaseShareCard({ file, shareOrigin }: { file: CaseFile; shareOrigin: string }) {
  ...
  return (
    <section className="card space-y-3" aria-labelledby="case-share-heading">
      ...
      {/*
        * flex-wrap + basis: ba nút vừa một hàng trên màn rộng, tự xuống dòng
        * trên điện thoại hẹp thay vì bóp chữ đến mức không đọc được.
        */}
      <div className="flex flex-wrap gap-2">
        <button
          className="btn-primary min-h-11 flex-1 basis-full sm:basis-0"
          onClick={handleShare}
          disabled={busy}
        >
          {busy ? "Đang dựng ảnh…" : "Chia sẻ hồ sơ"}
        </button>
        <button className="btn-secondary min-h-11 flex-1" onClick={handleDownload} disabled={busy}>
          Tải ảnh
        </button>
        <button className="btn-secondary min-h-11 flex-1" onClick={handleCopy} disabled={busy}>
          Sao chép tóm tắt
        </button>
      </div>

      {status && (
        <p className="text-xs text-mist/75" role="status" aria-live="polite">
          {status}
        </p>
      )}
...
function CardPreview({ model }: { model: CaseCardModel }) {
  ...
  return (
    <div
      className="mx-auto flex w-full max-w-[280px] flex-col overflow-hidden rounded-xl border border-white/10 bg-night-950"
